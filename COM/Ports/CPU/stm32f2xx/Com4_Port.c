...
/*-----------RCC------------------------------------------------------------------------------------------------------*/
    RCC_AHB1PeriphClockCmd( RCC_AHB1Periph_GPIOB, ENABLE);           /* 1.Enable GPIO(Tx,Rx pins) clock               */
   /*  ---pin not used in this application--- */                     /* 2.Enable  GPIO(RS485 TxRxDirection pin) clock */
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3,ENABLE);            /* 3.Enable USART clock                          */
/*-----------NVIC-----------------------------------------------------------------------------------------------------*/
...
/*-----------UART-----------------------------------------------------------------------------------------------------*/  
    ...
    USART_Init(USART3, &USART_InitStructure);                       /* 8. Set baudrate & frame format                 */        
    
    USART_ITConfig(USART3, USART_IT_RXNE, DISABLE);                 /* 9. Disable Rx&&Tx interrupt                    */          
    USART_Cmd(USART3, ENABLE);                                      /* 10.Enable U(S)ART  peripheral                  */
}
...
/*-----------RCC------------------------------------------------------------------------------------------------------*/  
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);                        /* 1. Enable TIM  clock    */
/*-----------NVIC-----------------------------------------------------------------------------------------------------*/  
...
*/
  ...
  TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);            /* 3.Set Timbase period accoding baudrate                */

  TIM_ClearITPendingBit(TIM4, TIM_IT_Update);                       
  TIM_ITConfig(TIM4, TIM_IT_Update, ENABLE);                 /* 4.Enable Timebase IT interrupt(clear before do it)    */

  TIM_Cmd(TIM4, DISABLE);                                    /* 5.DISABLE the TIM,(the core will hangle it self )     */            
}
