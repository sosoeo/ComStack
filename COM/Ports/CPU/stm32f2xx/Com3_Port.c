...
*          |                     Pin assignment                        |
*          +-----------------------------+---------------+-------------+
*          |  STM32F2xx USART6 ,TIM3     |    SN75176    |    Pin      |
...
/*-----------RCC------------------------------------------------------------------------------------------------------*/
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6,ENABLE);            /* 3.Enable USART clock                          */
...
/*-----------GPIO-----------------------------------------------------------------------------------------------------*/  
    ...
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_6;                     /* 5.Init GPIO Tx                                */
    GPIO_Init(GPIOC, &GPIO_InitStructure);
    
    GPIO_InitStructure.GPIO_Pin   = GPIO_Pin_7;                     /* 6.Init GPIO Rx                                */
    GPIO_Init(GPIOC, &GPIO_InitStructure);
...
/*-----------UART-----------------------------------------------------------------------------------------------------*/  
    ...
    USART_Init(USART6, &USART_InitStructure);                       /* 8. Set baudrate & frame format                 */        
    
    USART_ITConfig(USART6, USART_IT_RXNE, DISABLE);                 /* 9. Disable Rx&&Tx interrupt                    */          
    USART_Cmd(USART6, ENABLE);                                      /* 10.Enable U(S)ART  peripheral                  */
}
...
/*-----------RCC------------------------------------------------------------------------------------------------------*/  
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);                        /* 1. Enable TIM  clock    */
/*-----------NVIC-----------------------------------------------------------------------------------------------------*/  
...
*/
  ...
  TIM_TimeBaseInit(TIM3, &TIM_TimeBaseStructure);            /* 3.Set Timbase period accoding baudrate                */

  TIM_ClearITPendingBit(TIM3, TIM_IT_Update);                       
  TIM_ITConfig(TIM3, TIM_IT_Update, ENABLE);                 /* 4.Enable Timebase IT interrupt(clear before do it)    */

  TIM_Cmd(TIM3, DISABLE);                                    /* 5.DISABLE the TIM,(the core will hangle it self )     */            
}
...
static void UART_Tx_Message_by_DMA(T08U *buffer,T32U size) /* DMA data sending */
{
    ...
    DMA_InitStructure.DMA_PeripheralBaseAddr = ((uint32_t)USART6 + 0x04) ; /* USARTx_DR_ADDRESS */
